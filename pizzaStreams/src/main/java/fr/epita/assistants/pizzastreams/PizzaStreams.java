...
public class PizzaStreams {
    ...
    public static List<String> getVeggies(Stream<Pizza> pizzaStream) {
        return pizzaStream.filter(pizza -> pizza.getTopping().getVegetableList().stream().count() > 0 && pizza.getTopping().getProtein().isEmpty()).map(Pizza::getName).collect(Collectors.toList());
    }
    ...
    public static boolean areAllNatureItalians(Stream<Pizza> pizzaStream) {
        return pizzaStream.allMatch(p -> p.getDough() == Dough.NATURE && p.getTopping().getSauce() == Sauce.TOMATO && p.getTopping().getCheese() == Cheese.MOZZARELLA);
    }
}
